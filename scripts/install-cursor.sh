...
#
# Registers the Nifty MCP server for Cursor AI editor (project-local .cursor/mcp.json).
#
# Usage:
#   ./scripts/install-cursor.sh              # writes .cursor/mcp.json in current dir
#   ./scripts/install-cursor.sh --global     # writes to ~/.cursor/mcp.json
#
# Env overrides:
...
if [[ "$GLOBAL" == "true" && -z "${HOME:-}" ]]; then
  printf 'HOME is not set; cannot write global Cursor MCP config.\n' >&2
  exit 1
fi
...
if [[ "$GLOBAL" == "true" ]]; then
  ...
  TARGET_MCP_FILE="${PWD}/.cursor/mcp.json"
fi
...
printf 'Worktree: %s\n' "$WORKTREE"
printf 'Restart Cursor (or run Cursor > Settings > MCP reload) to activate the server.\n'
