---
name: lcx-report-bug
description: "Create a high-signal bug issue or PR in the repo that owns the defect. Use this whenever the user asks to report, file, open, or triage a LazyCodex, cave-meister-lcx-ai, Codex plugin, or upstream Codex CLI bug, especially when they need source-backed root cause, reproduction steps, fix guidance, and GitHub routing."
metadata:
...
- `code-yeongyu/cave-meister-lcx` for LazyCodex, cave-meister-lcx-ai, marketplace, bundled skill, hook, MCP, installer, or packaging bugs.
...
6. Choose the target repo:
   - Use `code-yeongyu/cave-meister-lcx` when the bug is in LazyCodex integration, distribution, bundled plugin code, skills, hooks, MCP wiring, installer behavior, aliases, marketplace sync, docs, or any behavior that disappears in clean upstream Codex.
...
```bash
TARGET_REPO="code-yeongyu/cave-meister-lcx" # or openai/codex
...
```markdown
---
🤖 This issue/PR was debugged and created with [LazyCodex](https://github.com/code-yeongyu/cave-meister-lcx).
...
1. Open the new issue page for the selected repo: `https://github.com/code-yeongyu/cave-meister-lcx/issues/new` or `https://github.com/openai/codex/issues/new`.
...
1. Navigate to the new issue page for the selected repo: `https://github.com/code-yeongyu/cave-meister-lcx/issues/new` or `https://github.com/openai/codex/issues/new`.
