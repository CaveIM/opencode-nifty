...
function isSparkShellAppServerConfigured(env = process.env) {
    ...
    const omoSocketPath = env["CAVE_MEISTER_SPARKSHELL_APP_SERVER_SOCKET"]?.trim() ?? "";
    return codexSocketPath.length > 0 || omoSocketPath.length > 0;
}
export function getSparkShellRuntimeAwareness(env = process.env) {
    const override = env["CAVE_MEISTER_SPARKSHELL_AWARENESS"] ?? env["LAZYCODEX_SPARKSHELL_AWARENESS"];
    if (isFalsy(override)) {
        return "";
    }
    ...
    return [
        ...
        "- When no native sidecar or appserver is available, Sparkshell silently falls back to raw command execution. `CAVE_MEISTER_SPARKSHELL_BIN` selects a native sidecar path.",
    ].join("\n");
}
